...
bool BoccoAPI::createSessions(){
  ...
  //アクセストークンを取得
  this->setAccessToken( jsonBuffer["access_token"] );
...
bool BoccoAPI::getFirstRoom(){
  ...
  auto error = deserializeJson(jsonBuffer, room);
  if (error) {
    return false;
  }

  this->firstRoomId = String((const char*) jsonBuffer["uuid"]);
...
bool BoccoAPI::postMessageText(const char* text){
  ...
  const String uid = ESPTrueRandom.uuidToString(uuidNumber);
